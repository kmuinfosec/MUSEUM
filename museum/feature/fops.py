...
def preprocess_func(file_path, use_bit_size):
    ...
    if len(data) >= 1:
        ...
        for func in range(len(data)):
            for xorContent in range(len(data[func])):
                ops = ''
                for contents in range(len(data[func][xorContent])):
                    ops += data[func][xorContent][contents]
                ops = ops.encode('utf-8')  # string -> bytes
                sha256_chunk = sha256(ops).digest()
                sha256_chunk = sha256bit_to_x_bit(sha256_chunk, use_bit_size)
                data[func][xorContent] = sha256_chunk
        for func in range(len(data)):
            xorFunc = 0
            for xorContent in range(len(data[func])):
                makeInt = int(data[func][xorContent], base=16)
                xorFunc = xorFunc ^ makeInt
            store_xor_func_data = str(hex(xorFunc))[2:]
            oneDemensionList.append(store_xor_func_data)
